...
- **Phase 5** — Production & Performance 🚧 Partially shipped out of order (Next.js migration, server-side cookie sessions, deployment, security headers all done; testing / TS / observability still pending)

The Next.js + `@supabase/ssr` migration is now on `main` as of 2026-05-01 — see the "Architecture note" at the top of this file. The class of bugs documented in Mistakes Log #15, #17, #18, #21, #23, #25-26 are resolved at the architecture level.
...
| 25 | Treated recurring symptoms as separate bugs instead of an architecture mismatch | Spent six commits patching tab-switch syncing, refresh-loses-page, click-binder-loops, missing-binders, and missing-cards independently. Each fix was correct in isolation but the same class of bug kept resurfacing because the root cause — browser-only Supabase client + localStorage tokens — was never addressed. | When the same shape of bug appears 3+ times in a row, stop patching and audit the architecture. The Vite-SPA-talking-to-Supabase-from-the-browser pattern has well-known race conditions; the canonical fix is server-side session validation (Next.js + `@supabase/ssr` cookies), now living on `feat/nextjs-migration`. |
| 26 | `ensureValidSession()` was misnamed — it didn't validate or refresh | The helper only called `supabase.auth.getSession()`, which reads localStorage and returns whatever's there. Services trusted its name and assumed they had a fresh token; in reality the token could be expired and PostgREST + RLS would silently return `[]`. The name created false confidence. | A function called `ensure*` or `validate*` must actually do the work. Either properly check `expires_at` and call `refreshSession()` when within an expiry buffer (commit `82d20ef`), or — preferred — push the responsibility to the platform (Next.js middleware does it on every request automatically). |
| 27 | Stacked caching layers without retiring the old ones | Profile cache in sessionStorage + binders cache in sessionStorage + per-binder cards cache in sessionStorage + React Query in-memory cache + Supabase localStorage. When any layer got stale, weird half-loaded states emerged ("'s Binders" header, empty cards on a real binder). Each round of fixes added a NEW cache key without retiring the OLD one. | One source of truth per data type. When introducing a new caching system (e.g. React Query), retire the old caches IN THE SAME PR. If you find yourself adding a `pkb_qcache:` namespace because the previous `pkb_cards_cache` had bugs, you've already lost. |
| 28 | Vite SPA shipped without a `/index.html` rewrite in `vercel.json` | Added URL routing (`/binder/:id`, `?page=N`) to the SPA in commit `a52d2d2` but never updated `vercel.json`. On a hard refresh of any deep URL Vercel's edge returned its own 404 (`sin1::pzltc-...`) before our app even loaded — `index.html` never ran, so React Router never saw the URL. Took a week of "refresh dumps me to main" reports to diagnose. | Whenever you introduce client-side routing on a static SPA, simultaneously add a catch-all rewrite to `vercel.json`: `{ "rewrites": [{ "source": "/((?!assets/|.*\\.(?:js\|css\|...)$).*)", "destination": "/index.html" }] }`. Or move to a real server framework (Next.js) and let it own routing. |
| 29 | Cwd drift across multiple worktrees in one shell session | Bash's working directory persists between calls, but I was juggling two worktrees (`wizardly-ishizaka-b5c8c5` and `next-migration`) and assumed each `cd` set things permanently. Some `cat > foo.css` and `mkdir -p` commands wrote to the wrong worktree silently — files appeared in the working tree of an unrelated branch. | When working in multiple worktrees in one session: (a) use absolute paths in Read/Write/Edit tools, (b) prefix EVERY Bash command with `cd /absolute/path && ...`, never trust the persisted shell cwd to match your current logical context. Add a `pwd &&` to the front of any command that has unclear cwd implications. |
| 30 | Committed `.next/` build output to git | The Next.js migration commit accidentally included 250+ generated files under `.next/` because the project's existing `.gitignore` (built for Vite) only ignored `dist/`. Made the diff unreadable and added MB of noise to the repo. | When migrating frameworks, audit `.gitignore` BEFORE the first build. Add framework-specific patterns (`.next/`, `out/`, `next-env.d.ts` for Next.js; `.turbo/`, `.svelte-kit/`, `.nuxt/` for others) on the same commit that introduces the framework. Verify with `git status --short` before committing. |
| 31 | Migrated framework, forgot to carry the global CSS bundle across | The Next.js migration created `src/app/globals.css` but the file was 0 bytes. The original Vite app's 2072 lines of styles (`src/App.css` + `src/index.css`) got deleted alongside the Vite entry files but never copied into `globals.css`. Production rendered as raw HTML — login page looked like a 1995 form. The build was green because empty CSS is valid CSS. | When deleting framework-specific entry files (`src/main.jsx`, `src/App.jsx`), audit what those files imported. CSS imports MUST be moved to the new framework's root layout, not just deleted. Diff `git show HEAD -- src/*.css` against the new globals to confirm. |
| 32 | Vercel project preset stuck on the old framework | After migrating from Vite to Next.js, Vercel's project setting still had `"framework": "vite"` — so production builds kept looking for `dist/` and failing with "No Output Directory named 'dist' found". Three production deploys ERROR'd in a row before the cause was clear from build logs. | Whenever you change `package.json#scripts.build` or move framework, also (a) declare `"framework": "<name>"` in `vercel.json` (which overrides the project preset), and (b) update Vercel Dashboard → Settings → Build & Development → Framework Preset to match. Either alone is fragile — vercel.json can be deleted, the dashboard preset can override on certain redeploy paths. |
| 33 | Pushed fixes to dev but never to main | Production was on commit `57a62c4` (pre-fix) for **3 deploys in a row** because every fix landed on dev/feat-branch and the user kept clicking "Redeploy" on the dashboard, which rebuilds the SAME (broken) commit. The user's "fix this" sessions got progressively more frustrated because the production URL never improved. | When the user reports a production bug, FIRST check `git log origin/main..origin/dev` — if there are commits on dev that haven't reached main, the user's "Redeploy" attempts are guaranteed to fail. Promote dev → main BEFORE asking the user to retry. |
| 34 | Browser bundle missing `NEXT_PUBLIC_*` env vars (Vite legacy variant only) | The Vercel project had `VITE_SUPABASE_URL` set since the Vite era. After the Next.js migration, the SERVER side worked (Node has `process.env` at runtime, so the legacy fallback in env.js resolved fine), but the BROWSER bundle had `undefined` because Next.js only inlines `NEXT_PUBLIC_*` variables at build time. Result: every browser fetch hit the placeholder URL. Server logs were clean, EnvHealthBanner didn't fire (it was a Server Component checking server-side env). Looked like a runtime bug. | When migrating frameworks, AUDIT and RENAME env vars to match the new framework's convention before declaring the migration done. For mixed setups, the EnvHealthBanner must check the EXACT names that get inlined into the browser bundle, not the server-side fallback. Code-level: prefer one canonical name and fail loudly when missing, rather than silently falling back to a name the browser can't see. |
| 35 | Cross-table RLS policies caused infinite recursion on read | Migration 003 added `profiles_public_select` (subquery on `binders`) alongside the existing `binders_select` (subquery on `profiles`). Postgres OR's all matching policies — so a SELECT on `profiles` triggered evaluation of `binders` RLS, which triggered evaluation of `profiles` RLS, infinitely. Every `/rest/v1/profiles` request returned 500 with `infinite recursion detected in policy for relation "profiles"`. UI symptom: dashboard hung on the spinner forever, UserMenu fell back to email-prefix because profile state stayed null. | When writing RLS policies that cross-reference another table, use a `SECURITY DEFINER STABLE` helper function (with `SET search_path = ''`) in the USING clause. The function runs with the function-owner's privileges, bypassing the caller's RLS for that lookup — the cycle terminates. Pattern: `CREATE FUNCTION public.is_x_y(_id) RETURNS boolean LANGUAGE sql SECURITY DEFINER STABLE SET search_path = '' AS $$ SELECT EXISTS(SELECT 1 FROM ...) $$;` then `USING (public.is_x_y(col))`. |
| 36 | Sized icon button without resetting global button padding | View-mode toggle buttons (`view-mode-btn`) in BinderView were styled with `width: 34px; height: 30px`, but the global `button { padding: 0.6em 1.2em }` from the legacy `index.css` reset persisted. With `box-sizing: border-box`, ~38px of horizontal padding inside a 34px-wide button collapsed the content area to zero. Icons rendered but had nowhere to go — visible empty rectangle. | When applying explicit width/height to a `<button>`, ALWAYS also set `padding: 0` (or an appropriate small value) explicitly. The browser/global default padding will sneak in via `box-sizing: border-box` and silently eat the content area. Same applies to inputs with explicit dimensions. |

---

## What worked (keep doing)

A running log of patterns that *did* solve problems on this codebase. Future sessions should reach for these first when similar issues arise.

| # | Pattern | Why it worked |
|---|---|---|
| A | URL is the source of truth for view state | Replacing the sessionStorage view-cache with `useParams` + `useSearchParams` made hard-refresh restoration bulletproof. The URL can't desync from itself. Don't shadow URL-derivable state in component state or storage. |
| B | TanStack Query for client-side server state | Replaced ~150 lines of hand-rolled `useState` + `useEffect` + `setSyncing` flag-juggling with declarative hooks. Built-in `refetchOnWindowFocus`, `refetchOnReconnect`, retry, optimistic updates with rollback. The same library that production React apps reach for, and it just works. |
| C | Single-flight refresh pattern | One in-flight `refreshSession()` promise shared by all callers (used by both the 401 interceptor in `supabase.js` and the bootstrap in AuthContext). Concurrent 401s from multiple queries collapse into ONE refresh, not N. Cheap to implement, eliminates a whole category of races. |
| D | `@supabase/ssr` with cookie-based sessions | Moves session lifecycle from "browser JS hopes the timer fires" to "server validates on every request". Eliminates background-tab throttling, cross-tab sync, and stale-JWT-empty-RLS at the source. The migration is on `feat/nextjs-migration` — this is the right architecture for production Supabase apps. |
| E | Worktrees for risky refactors | Created `next-migration` worktree from `dev` and worked there in parallel. The existing dev preview kept working untouched while the Next.js migration was built. Lets the user keep using the app while big architectural changes are tested. Prefer this over feature branches in the same worktree for any refactor that touches `package.json` / framework files. |
| F | The Mistakes Log itself | Multiple regressions (#15, #17, #21) only got caught because the entry that warned about them was already in this file. Future sessions: read the Mistakes Log before debugging auth, sessions, or RLS. The pattern that's biting you is probably already documented. |
