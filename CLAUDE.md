...
| 17 | Supabase RLS returns empty data (not errors) on expired JWT | When JWT expires and auto-refresh hasn't fired (browser throttles timers in background tabs), all RLS queries silently return `[]`. Code treated this as "user has no binders" → blank dashboard. | Service layer (`binderService`, `cardService`) now detects empty-but-suspicious results and calls `ensureValidSession()` to force a token refresh before retrying. |
| 18 | No session refresh on tab visibility change | Supabase `autoRefreshToken` uses `setTimeout`, which browsers throttle in background tabs. User leaves tab for >1 hour → JWT expires → all DB queries fail silently. | Added `visibilitychange` listener in AuthContext that calls `getSession()` (triggers refresh) when tab regains focus. Also added 4-minute periodic health check. |
